...
def _convolve(kwargs:dict):
    m_start, m_stop = kwargs['ind']
    A_prev_pad = kwargs['A_prev_pad']
    n_H = kwargs['n_H']
    n_W = kwargs['n_W']
    n_C = kwargs['n_C']
    ...
    Z = np.zeros((m_stop-m_start, n_H, n_W, n_C))
    for i in range(m_start, m_stop):  # loop over the batch of training examples
        ...
        for h in range(n_H):  # loop over vertical axis of the output volume
            for w in range(n_W):  # loop over horizontal axis of the output volume
                for c in range(n_C):  # loop over channels (= #filters) of the output volume
                    ...
                    Z[i-m_start, h, w, c] = conv_single_step(a_slice_prev, W[..., c])
...
class Conv2d(Function):
    ...
    def _forward(self) -> np.ndarray:
        ...
        for i in range(threads_num):
            j = int(m / threads_num)
            dict_ind = {}
            dict_ind['ind'] = ((i * j, (i + 1) * j))
            dict_ind['A_prev_pad'] = A_prev_pad
            dict_ind['n_H'] = n_H
            dict_ind['n_W'] = n_W
            dict_ind['n_C'] = n_C
            dict_ind['f'] = f
            dict_ind['stride'] = stride
            dict_ind['W'] = W
            dict_list.append(dict_ind)

        l = Parallel(n_jobs=4)(delayed(_convolve)(i) for i in dict_list)
        #_convolve(dict_list[0])
        # Making sure your output shape is correct
        Z1 = np.zeros((m, n_H, n_W, n_C))

        for i,d in enumerate(dict_list):
            m1, m2 = d['ind']
            Z1[m1:m2] = l[i]
        assert (Z1.shape == (m, n_H, n_W, n_C))
            ...
            for h in range(n_H):  # loop over vertical axis of the output volume
                for w in range(n_W):  # loop over horizontal axis of the output volume
                    for c in range(n_C):  # loop over the channels of the output volume
                        ...
                        da_prev_pad[vert_start:vert_end, horiz_start:horiz_end, :] += W[:, :, :, c] * grad_output[
                            i, h, w, c]
